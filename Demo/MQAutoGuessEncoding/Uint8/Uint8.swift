...
extension Data {
    ...
    fileprivate func model_uint8() -> MQAutoGuessEncoding_uint8? {
        let bundle = Bundle(for: MQAutoGuessEncoding_uint8.self)
        if bundle.url(forResource: "MQAutoGuessEncoding_uint8", withExtension:"mlmodelc") != nil {
            return MQAutoGuessEncoding_uint8()
        }
        
        guard let modelPath = bundle.path(forResource: "MQAutoGuessEncoding_uint8", ofType: "mlmodel") else {
            return nil
        }
        ...
        guard let model = try? MQAutoGuessEncoding_uint8.init(contentsOf: mlmodelcURL) else {
            return nil
        }
    ...
    fileprivate func cntAry_uint8() -> [Double] {
        let len = Int(UInt8.max) + 1
        var cntAry = Array.init(repeating: 0.0, count: len)
        ...
        for byte in self {
            ...
            cnt += 1
            cntAry[Int(byte.byteSwapped)] += 1
        }
        
        return cntAry
    }
    
    fileprivate func mq_autoGuessEncoding_uint8() -> String.Encoding? {
        var tsModel: MQAutoGuessEncoding_uint8?
        var tsCntAry: [Double]?
        DispatchQueue.concurrentPerform(iterations: 2) { (i) in
            if i == 0 {
                tsModel = self.model_uint8()
                return
            }
            
            tsCntAry = self.cntAry_uint8()
        }
    ...
    public func mq_autoString_uint8() -> String? {
        guard let encoding = self.mq_autoGuessEncoding_uint8() else {
            return nil
        }
