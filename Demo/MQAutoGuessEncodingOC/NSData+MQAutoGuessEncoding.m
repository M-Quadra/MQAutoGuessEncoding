...
#import "NSData+MQAutoGuessEncoding.h"
#import "MQAutoGuessEncoding_uint4xuint4.h"

API_AVAILABLE(macos(10.13), ios(11.0), watchos(4.0), tvos(11.0)) __attribute__((visibility("hidden")))
@implementation NSData (MQAutoGuessEncoding)

- (MQAutoGuessEncoding_uint4xuint4 *)mq_model_uint4xuint4 {
    NSBundle *bundle = [NSBundle bundleForClass:MQAutoGuessEncoding_uint4xuint4.class];
    NSURL *mlmodelcURL = [bundle URLForResource:@"MQAutoGuessEncoding_uint4xuint4" withExtension:@"mlmodelc"];
    if (mlmodelcURL) {
        return [MQAutoGuessEncoding_uint4xuint4 new];
    }
    
    NSString *modelPath = [bundle pathForResource:@"MQAutoGuessEncoding_uint4xuint4" ofType:@"mlmodel"];
    ...
    return [[MQAutoGuessEncoding_uint4xuint4 alloc] initWithContentsOfURL:mlmodelcURL error:nil];
}

- (NSStringEncoding)mq_autoGuessEncoding {
    const int len = 16;
    double cntAry[len][len];
    memset(cntAry, 0, sizeof cntAry);
    double (* cntAryPtr)[len][len] = &cntAry;
    
    __block MQAutoGuessEncoding_uint4xuint4 *model = nil;
    
    dispatch_apply(2, dispatch_get_global_queue(0, 0), ^(size_t i) {
        if (i == 0) {
            model = [self mq_model_uint4xuint4];
            return;
        }
    ...
    MQAutoGuessEncoding_uint4xuint4Output *result = [model predictionFromC0x0: cntAry[0][0] c0x1: cntAry[0][1] c0x2: cntAry[0][2] c0x3: cntAry[0][3] c0x4: cntAry[0][4] c0x5: cntAry[0][5] c0x6: cntAry[0][6] c0x7: cntAry[0][7] c0x8: cntAry[0][8] c0x9: cntAry[0][9] c0x10: cntAry[0][10] c0x11: cntAry[0][11] c0x12: cntAry[0][12] c0x13: cntAry[0][13] c0x14: cntAry[0][14] c0x15: cntAry[0][15]
    ...
    c14x0: cntAry[14][0] c14x1: cntAry[14][1] c14x2: cntAry[14][2] c14x3: cntAry[14][3] c14x4: cntAry[14][4] c14x5: cntAry[14][5] c14x6: cntAry[14][6] c14x7: cntAry[14][7] c14x8: cntAry[14][8] c14x9: cntAry[14][9] c14x10: cntAry[14][10] c14x11: cntAry[14][11] c14x12: cntAry[14][12] c14x13: cntAry[14][13] c14x14: cntAry[14][14] c14x15: cntAry[14][15]
                         c15x0: cntAry[15][0] c15x1: cntAry[15][1] c15x2: cntAry[15][2] c15x3: cntAry[15][3] c15x4: cntAry[15][4] c15x5: cntAry[15][5] c15x6: cntAry[15][6] c15x7: cntAry[15][7] c15x8: cntAry[15][8] c15x9: cntAry[15][9] c15x10: cntAry[15][10] c15x11: cntAry[15][11] c15x12: cntAry[15][12] c15x13: cntAry[15][13] c15x14: cntAry[15][14] c15x15: cntAry[15][15] error:nil];
    
    if (result == nil) {
        return NSUTF8StringEncoding;
    }
    
    NSString *codingStr = [result.coding stringByReplacingOccurrencesOfString:@"c" withString:@""];
    return [codingStr longLongValue];
}

- (nullable NSString *)mq_autoString {
    NSStringEncoding encoding = [self mq_autoGuessEncoding];
